<?php
require 'includes/checkforlogin.php';
include 'class/db.inc.php';
include 'class/exe.inc.php';
$obj=new logincheck;
if($obj->CheckForLogin())
  header('location:member/backend.php');
?>
<!DOCTYPE html>
...
<?php if (isset($_POST['username']) && isset($_POST['passw'])): ?>
input[type=password] {
  padding: 10px;
  ...
  <body>
    ...
    <div class="login">
      <?php
          ...
          {
          ?>
          <form method="POST" name="sec" action="member/backend.php">
          <input type="hidden" name="username" value="<?php echo $_POST['username']; ?>">
          <input type="hidden" name="passw" value="<?php echo $_POST['pass']; ?>">
          <table>
            <tr>
              <th><input type="password" name="t1" id="t1" onkeyup="check()" required="required" maxlength="1"></th>
              <th><input type="password" name="t2" id="t2" onkeyup="check1()" required="required" maxlength="1"></th>
              <th><input type="password" name="t3" id="t3" onkeyup="check2()" required="required" maxlength="1"></th>
              <th><input type="password" name="t4" required="required" maxlength="1"></th>
            </tr>
          </table>
        ...
        <input type="password" id="passw" name="pass" placeholder="Password" required="required" onmouseover="showpass()" onmouseout="hidepass()" />
        <button type="submit" class="btn btn-primary btn-block btn-large">Login</button>
        </form>
        <?php
          }
